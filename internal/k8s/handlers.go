	...
	return cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			if !secrets.IsSupportedSecretType(secret.Type) {
				glog.V(4).Infof("Ignoring Secret %v of unsupported type %v", secret.Name, secret.Type)
				return
			}
		...
		UpdateFunc: func(old, cur interface{}) {
			...
			if !secrets.IsSupportedSecretType(curSecret.Type) {
				glog.V(4).Infof("Ignoring Secret %v of unsupported type %v", curSecret.Name, curSecret.Type)
				return
			}
